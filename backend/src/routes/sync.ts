...
export async function syncRoutes(fastify: FastifyInstance) {
  fastify.get('/sync', async (request, reply) => {
    ...
    const agenda = await prisma.agendaEvent.findMany({
    ...
    const initiatives = sinceDate
      ? []
      : await prisma.initiative.findMany({ orderBy: { displayOrder: 'asc' } })
    const sponsors = sinceDate
      ? []
      : await prisma.sponsor.findMany({ orderBy: { displayOrder: 'asc' } })
